...
class OTPVerificationView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not token:
            messages.error(
                request, "Invalid or missing token. Please request a new OTP."
            )
            return redirect("forgot_password")
    ...
    def resend_otp(self, request):
        ...
        if email:
            otp = self.user_service.generate_and_send_otp(
                email=email, token=token, resend=True
            )
            if otp:
...
class ResetPasswordView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not token:
            messages.error(
                request, "Invalid or missing token. Please generate OTP first."
            )
            return redirect("forgot_password")
        ...
        try:
            ...
            if not payload:
                ...
                return redirect(
                    "forgot_password"
                )  # Redirect to forgot password view if token is invalid
            ...
            if not email:
                ...
                return redirect(
                    "forgot_password"
                )  # Redirect to forgot password view if email is missing in the payload
            ...
            return redirect(
                "forgot_password"
            )  # Redirect to forgot password view if the token is expired
        except jwt.InvalidTokenError:
            messages.error(request, "Invalid token.")
            return redirect(
                "forgot_password"
            )  # Redirect to forgot password view if the token is invalid
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.error(
                f"Error occurred while resetting password for email {email}: {str(e)}"
            )
            messages.error(
                self.request,
